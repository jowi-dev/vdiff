...
//! 3. **X-assignment**: once ordering is fixed, each band is laid out
//!    left-to-right in char units: `x` is the running offset, `width` is
//!    the slot's label length (`1` for a dummy, which renders as a bare
//!    routing point with no label at all).
//!
